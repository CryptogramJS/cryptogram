...
(() => {
    ...
    function keysFromSeed(seed) {
        ...
        const sym = sodium.crypto_generichash(
            32,
            sodium.from_string('personal_blob_key_derivation'),
            seed
        );
    ...
    const lockPersonal = () => {
        ...
        const c = sodium.crypto_secretbox_easy(
            sodium.from_string(JSON.stringify(personal)),
            n,
            secretKey
        );
    ...
    async function putPersonalBlob() {
        ...
        try {
            ...
            if (!r.ok) {
                throw new Error(
                    `HTTP error saving personal blob: ${r.statusText} (Status: ${r.status})`
                );
            }
        } catch (e) {
    ...
    async function newWebhookSession() {
        try {
            ...
            if (!r.ok) {
                throw new Error(
                    `Error creating webhook session: ${r.statusText} (Status: ${r.status})`
                );
            }
            hookSlug = await r.text();
    ...
    async function otsStamp(hashHex) {
        if (!CFG.ots_stamp_endpoint) {
            console.warn('OTS stamp endpoint is not configured.');
            return null;
        }
        try {
            ...
            if (!r.ok) {
                console.error(
                    `Error stamping OTS: ${r.statusText} (Status: ${r.status})`
                );
                return null;
            }
    ...
    async function otsVerify(hashHex) {
        if (!CFG.ots_proof_endpoint) {
            console.warn('OTS proof endpoint is not configured.');
            return false;
        }
        try {
            ...
            if (!r.ok) {
                console.warn(
                    `Error verifying OTS: ${r.statusText} (Status: ${r.status})`
                );
                return false;
            }
    ...
    function amChange(url, fn) {
        ...
        if (!Automerge.equals(prevHeads, Automerge.getHeads(ctx.doc))) {
            ...
            ctx.dirty = true;
            console.log(`Change applied and chat marked as dirty for ${url}.`);
        }
    }
    ...
    async function flushChat(url) {
        ...
        await syncChat(url);
        if (!ctx.dirty) {
            console.log(`Flush canceled for ${url} - no local changes after sync.`);
            return;
        }
        ...
        try {
            ...
            if (remoteJson && !safeToPush(ctx.meta, remoteJson.meta)) {
                console.warn(`Not safe to push changes for ${url}. Another change detected recently. Will retry on next sync.`);
                ctx.dirty = true;
                return;
            }
        } catch (e) {
        ...
        try {
            ...
            if (!r.ok) {
                throw new Error(`HTTP error on flush: ${r.statusText} (Status: ${r.status})`);
            }
    ...
    async function syncChat(url) {
        ...
        try {
            const r = await fetch(url);
            if (r.ok) remoteJson = await r.json();
        } catch (e) {
        ...
        if (
            remoteJson.meta.lastModified > ctx.meta.lastModified ||
            ...
            try {
                ...
                console.error(`Error merging during sync for ${url}:`, e);
            }
        } else {
        ...
        return ctx.doc.log
            .map(l => {
                try {
                    ...
                    const calc = sodium.to_base64(
                        sodium.crypto_generichash(32, sodium.from_base64(c), ctx.key)
                    );
                    if (mac !== calc) {
                        console.warn(`MAC mismatch for a message in chat ${url}. Message corrupted or altered.`);
                        return null;
                    }
                    const p = sodium.crypto_secretbox_open_easy(
    ...
    async function verifySnapshot(chatUrl, ts) {
        try {
            ...
            if (!ctx) {
                console.error(`Missing chat context for verifySnapshot: ${chatUrl}`);
                return false;
            }
            ...
            amChange(chat.chat_url, d => {
                ...
                if (chat.peerSlug) {
                    d.meta[chat.peerSlug] = {
                        http: `${CFG.webhook_base_url}/${chat.peerSlug}`,
    ...
    async function createAccount() {
        try {
            ...
            personal = {
                ...
                session: {
                    issued: Date.now(),
                    expires: Date.now() + CFG.token_lifetime_ms,
                    last_login: null,
                    last_logout: null
                },
                chats: []
            ...
            if (!r.ok) {
                throw new Error(
                    `Error creating initial blob: ${r.statusText} (Status: ${r.status})`
                );
            }
            blobUrl = r.headers.get('Location').replace(/^http:/, 'https:');
            await putPersonalBlob();
            cacheProfile();
            console.log('Account successfully created. Blob URL:', blobUrl);
            return sodium.to_base64(
                sodium.from_string(
                    JSON.stringify({ entropy: entropyHex, slug: blobUrl.split('/').pop() })
                )
            );
        } catch (e) {
    ...
    async function authenticate(ext) {
        try {
            const { entropy, slug } = JSON.parse(
                sodium.to_string(sodium.from_base64(ext))
            );
            ...
            personal = unlockPersonal(await fetch(blobUrl).then(r => {
                if (!r.ok) throw new Error(
                    `Error fetching personal blob: ${r.statusText} (Status: ${r.status})`
                );
                return r.json();
            }));
    ...
    async function sendInvite(friendSlug) {
        try {
            ...
            if (!r.ok) {
                throw new Error(
                    `Error creating invite blob: ${r.statusText} (Status: ${r.status})`
                );
            }
            ...
            personal.chats.push({ chat_url: chatUrl, key_hex: keyHex, peerSlug: null,_peerEmail: null, nickname: null });
            await putPersonalBlob();
            cacheProfile();
            const invite = { type: 'invite', chatUrl, keyHex, ts: ts_hex, fromSlug: hookSlug, fromEmail: hookEmail };
            await fetch(`${CFG.webhook_base_url}/${friendSlug}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(invite)
            }).catch(e => console.warn('Error sending invite webhook:', e));
            console.log(`Invite sent to ${friendSlug} for chat ${chatUrl}.`);
        } catch (e) {
            ...
            switch (msg.type) {
                case 'invite': {
                    if (!personal.chats.some(c => c.chat_url === msg.chatUrl)) {
                        ...
                        await putPersonalBlob();
                        cacheProfile();
                        console.log(`Invite accepted and chat added: ${msg.chatUrl}`);
                    } else {
                        console.warn(`Duplicate invite for chat ${msg.chatUrl} ignored.`);
                    }
                ...
                case 'accepted': {
                    ...
                    if (c) {
                        ...
                        await putPersonalBlob();
                        cacheProfile();
                        console.log(`Invite accepted by ${msg.bySlug} for chat ${msg.chatUrl}.`);
                    } else {
                        console.warn(`Chat ${msg.chatUrl} not found to mark acceptance.`);
                    }
                    break;
                }
                case 'flush':
    ...
    async function sendMessage(chat, txt) {
        try {
            ...
            const cipher = sodium.crypto_secretbox_easy(
                sodium.from_string(messagePayload),
                n,
                ctx.key
            );
    ...
    async function logout() {
        try {
            ...
            console.log('Complete logout and local data cleared.');
        } catch (e) {
