...
(() => {
 ...
 Object.defineProperties(_state, {
    ...
    hookApiToken:{ get: () => hookApiToken,set: v => hookApiToken = v },
 ...
 const lockPersonal = () => {
    ...
    const c = sodium.crypto_secretbox_easy(sodium.from_string(JSON.stringify(personal)), n, secretKey);
    return { nonce: sodium.to_base64(n), ciphertext: sodium.to_base64(c) };
 };
 const unlockPersonal = enc => {
    const p = sodium.crypto_secretbox_open_easy(sodium.from_base64(enc.ciphertext), sodium.from_base64(enc.nonce), secretKey);
 ...
 async function newWebhookSession() {
    ...
    _state.hookSlug = j.uuid;
    if (j.token) {
      _state.hookApiToken = j.token;
      localStorage.setItem("0k_webhook_api_token", j.token);
    }
    _state.hookUrl = `https://webhook.site/${_state.hookSlug}`;
    _state.hookEmail = `${_state.hookSlug}@${CFG.webhook_email_domain}`;
    localStorage.setItem("0k_webhook_slug", _state.hookSlug);
    localStorage.setItem("0k_webhook_url",  _state.hookUrl);
    localStorage.setItem("0k_webhook_email",_state.hookEmail);
    document.dispatchEvent(new CustomEvent("ok0:newSlug", {
      detail: {
        slug: _state.hookSlug,
        http: _state.hookUrl,
        email: _state.hookEmail,
        apiToken: _state.hookApiToken
      }
    }));
 }
 ...
 async function authenticate(ext) {
    ...
    const storedSlug     = localStorage.getItem("0k_webhook_slug");
    const storedApiToken = localStorage.getItem("0k_webhook_api_token");
    const storedUrl      = localStorage.getItem("0k_webhook_url");
    const storedEmail    = localStorage.getItem("0k_webhook_email");

    if (storedSlug && storedApiToken && storedUrl && storedEmail) {
      _state.hookSlug     = storedSlug;
      _state.hookApiToken = storedApiToken;
      _state.hookUrl      = storedUrl;
      _state.hookEmail    = storedEmail;
      document.dispatchEvent(new CustomEvent("ok0:newSlug", {
        detail: {
          slug: _state.hookSlug,
          http: _state.hookUrl,
 ...
 async function logout() {
    ["0k_webhook_slug","0k_webhook_url","0k_webhook_email","0k_webhook_api_token","0k_token","0k_token_exp","0k_blob_url","0k_username"]
