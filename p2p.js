...
(() => {
 ...
 async function flushChat(url) {
  ...
  ctx.dirty = false;
  try {
    await fetch(url, { method:"PUT", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ bin:Array.from(finalBin), meta:ctx.meta }) });
  } catch {
    ctx.dirty = true;
  }
 }
 ...
 async function acceptInvite(chatUrl, inviterSlug, keyHex, inviterPubKey) {
  ...
  if (chat) {
    ...
    await fetch(getPeerWebhookUrl(inviterSlug, 'accepted'), {
      method:"POST",
      headers:{ 'Content-Type':'application/json' },
      body:JSON.stringify({ type:"accepted", chatUrl, bySlug: S.hookSlug, byEmail: S.hookEmail, byPubKey: S.myPubKeyHex })
    }).catch(()=>{});
  }
 }
  ...
  switch(msgType) {
    case "invite": {
      ...
      if (!S.personal.chats.some(c => c.chat_url === chatUrl)) {
        S.personal.chats.push({ chat_url:chatUrl, key_hex:keyHex, peerSlug:fromSlug, peerEmail:fromEmail, nickname:null, accepted:false, iInitiated:false, peerPubKey:fromPubKey });
        await ok0.putPersonalBlob(); ok0.db.profile.put({ key:"me", data:S.personal });
