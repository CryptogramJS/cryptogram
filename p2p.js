/*****************************************************************
 * p2p.js – invites, messaging, presence, TSA stamping (final)
 *****************************************************************/
(() => {
  ...
  /* SHORTCUTS */
  const { cfg: CFG, utils, _state: S } = ok0;

  /* WEBHOOK ROUTES */
  // Only initial creation uses proxy (webhook_create_endpoint)
  const DIRECT_BASE = CFG.webhook_base_url_direct;      // e.g. https://webhook.site
  const MY_ENDPOINT = () => `${DIRECT_BASE}/${S.hookSlug}`;
  const PEER_ENDPOINT = slug => `${DIRECT_BASE}/${slug}`;

  /* CONSTANTS */
  const chatDocs     = {};
  const DEF_SYNC_MS  = CFG.chat_sync_interval_ms || 3000;
  ...
  async function initChatDoc(url, keyHex) {
    ...
    const interval = setInterval(() => syncChat(url), DEF_SYNC_MS);
  ...
  async function syncChat(url) {
    ...
    const needMerge = remote.meta.lastModified > ctx.meta.lastModified
    ...
    if (rec) {
      const newSlug = findLatestPeerSlug(ctx.meta, rec.peerSlug);
      if (newSlug && newSlug !== rec.peerSlug) {
        rec.peerSlug  = newSlug;
        rec.peerEmail = `${newSlug}@${CFG.webhook_email_domain}`;
  ...
  async function flushChat(url) {
    const ctx = chatDocs[url]; if (!ctx?.dirty) return;
    await syncChat(url);
    if (!ctx.dirty) return;
    ...
    try {
      ...
      if (r.ok) {
        ...
        if (!safeToPush(ctx.meta, remote.meta)) { ctx.dirty = true; return; }
      }
    } catch {}
  ...
  function flushPending(chat) {
    ...
    while (q.length) {
      const enc = q.shift();
      fetch(`${PEER_ENDPOINT(chat.peerSlug)}`, {
        method:'POST',
        headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('0k_token')}` },
  ...
  function findLatestPeerSlug(meta, current) {
    ...
    for (const e of Object.values(meta || {})) {
      if (!e.slug || e.slug === current) continue;
      if (!best || (e.ts || 0) > (best.ts || 0)) best = e;
    }
    return best?.slug || null;
  }

  /* API: invites, messages */
  async function ensureChat(chat) {
    await initChatDoc(chat.chat_url, chat.key_hex);
    amChange(chat.chat_url, d => {
      d.meta = d.meta || {};
      d.meta[S.myPubKeyHex] = { slug:S.hookSlug, http:MY_ENDPOINT(), email:S.hookEmail, ts:Date.now() };
      if (chat.peerSlug) {
        d.meta[chat.peerSlug] = { slug:chat.peerSlug, http:PEER_ENDPOINT(chat.peerSlug), email:chat.peerEmail, ts:Date.now() };
      }
    });
  }
  ...
  async function sendInvite(friendSlug) {
    ...
    if (!r.ok) throw new Error('Invite create failed');
    const chatUrl = r.headers.get('Location').replace(/^http:/,'https:');
    const ts_hex  = Date.now().toString(16);
    const keyHex  = await utils.sha512(new TextEncoder().encode(ts_hex+'chat'))
    ...
    await fetch(`${PEER_ENDPOINT(friendSlug)}`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('0k_token')}` },
  ...
  async function declineInvite(chatUrl, inviterSlug) {
    await fetch(`${PEER_ENDPOINT(inviterSlug)}`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('0k_token')}` },
  ...
  async function sendMessage(chat, txt) {
    ...
    const ctx = chatDocs[chat.chat_url]; if (!ctx?.key) throw new Error('Missing chat key');
    const n = sodium.randombytes_buf(24);
    const pl = JSON.stringify({ from:S.myUsername, txt, ts:Date.now() });
    const cipher = sodium.crypto_secretbox_easy(sodium.from_string(pl), n, ctx.key);
    ...
    const url   = `${PEER_ENDPOINT(chat.peerSlug)}`;
    if (chat.peerSlug && navigator.onLine) {
      fetch(url, { method:'POST', headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('0k_token')}`} , body:JSON.stringify({ type:'message', chatUrl:chat.chat_url, payload:enc }) })
  ...
  function getChatMessages(url) {
    ...
    if (!ctx?.doc?.log || !ctx.key) return [];
    return ctx.doc.log.map(row => {
      try {
        const [n,c,m] = row.split('.');
        const check = sodium.to_base64(sodium.crypto_generichash(32, sodium.from_base64(c), ctx.key));
        if (m !== check) return null;
        const pt = sodium.crypto_secretbox_open_easy(sodium.from_base64(c), sodium.from_base64(n), ctx.key);
  ...
  async function verifySnapshot(chatUrl, ts) {
    ...
    const tsr = ctx.doc.meta?.tsa?.flush?.[ts];
    return Array.isArray(tsr) && tsr.length>0;
  }
  ...
  async function onIncomingWebhook(msg) {
    switch(msg.type) {
      case 'invite': await handleInvite(msg); break;
      case 'accepted': await handleAccepted(msg); break;
      case 'declined': await handleDeclined(msg); break;
      case 'flush': await flushChat(msg.chatUrl); break;
      case 'message': await handleMessage(msg); break;
    }
  }
  ...
  async function handleInvite(msg) {
    ...
    fetch(`${PEER_ENDPOINT(msg.fromSlug)}`, {
      method:'POST',
      headers:{ 'Content-Type':'application/json', 'Authorization': `Bearer ${localStorage.getItem('0k_token')}`},
      body: JSON.stringify({ type:'accepted', chatUrl:msg.chatUrl, bySlug:S.hookSlug, byEmail:S.hookEmail })
    }).catch(()=>{});
  }

  async function handleAccepted(msg) {
    const c = S.personal.chats.find(c=>c.chat_url===msg.chatUrl);
    if (c) {
  ...
  async function handleDeclined(msg) {
    const idx = S.personal.chats.findIndex(c=>c.chat_url===msg.chatUrl);
    if (idx!==-1) {
      const ch = S.personal.chats[idx];
      if (!ch.peerSlug) {
        S.personal.chats.splice(idx,1);
        delete pending[ch.chat_url]; delete pendingOffline[ch.chat_url];
        await ok0.putPersonalBlob(); ok0.db.profile.put({key:'me',data:S.personal});
      }
    }
  }
  ...
  async function handleMessage(msg) {
    await initChatDoc(msg.chatUrl, msg.keyHex || null);
  ...
  (async function poll() {
    for (;;) {
      if (!S.hookSlug) { await new Promise(r=>setTimeout(r,2000)); continue; }
      try {
        const url = `${MY_ENDPOINT()}/requests?min_id=${lastPoll}&sort=asc&limit=20`;
        const r = await fetch(url, { cache:'no-store', headers:{ 'Authorization': `Bearer ${localStorage.getItem('0k_token')}`} });
        if (r.ok) {
          const list = await r.json();
          for (const it of list) {
            lastPoll = Math.max(lastPoll, it.id);
            try { await onIncomingWebhook(JSON.parse(it.content)); } catch {};
          }
        }
      } catch {}
  ...
  window.addEventListener('online', async ()=>{
    ...
    for (const ctx of Object.values(chatDocs)) if (ctx.dirty) await flushChat(ctx.docUrl);
  });

  /* EXPORT */
  Object.assign(ok0, { ensureChat, sendInvite, declineInvite, sendMessage, closeChat, getChatMessages, verifySnapshot, onIncomingWebhook });
