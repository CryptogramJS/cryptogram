...
function showToast(msg, type='info'){
  ...
  if(!box) return console.error('Toast container missing');
...
async function genCap(img){
  ...
  img.src=c.toDataURL(); return sha256(s);
}
async function refreshCaps(){ hLog=await genCap($('#cap-img-log')); hReg=await genCap($('#cap-img-reg')); }
function capPass(type,ok){
  ...
  if(block>now){showToast(`Blocked ${Math.ceil((block-now)/1000)}s`,'warning');return false;}
  if(ok){ if(type==='reg')triesR=0; else triesL=0; return true; }
  type==='reg'?triesR++:triesL++;
  const cur = type==='reg'?triesR:triesL;
  if(cur>=max){ type==='reg'?blkR=now+cool:blkL=now+cool; triesR=triesL=0; showToast(`Blocked ${cool/1000}s`,'danger');}
  else showToast(`Captcha wrong. Left ${max-cur}`,'warning');
  return false;
}
refreshCaps();
...
$('#btn-reg').onclick = async ()=>{
  ...
  try{
    ...
    $('#extended-out').value=tok; $('#reg-sec').classList.remove('d-none');
    window._tok=tok; showToast('Account created – save the token.','success');
  }catch(e){ showToast(e.message,'danger');}
  toggleLoading($('#btn-reg'),false); refreshCaps();
};
$('#btn-reg-ok').onclick = async ()=>{
  const t=$('#extended-out').value.trim();
  try{ await navigator.clipboard.writeText(t); showToast('Token copied.','success');}
  catch{ $('#extended-out').select(); document.execCommand('copy'); showToast('Token copied.','success');}
...
async function loginFlow(){
  ...
  try{
    const u=await authenticate(tok); showToast(`Welcome ${u}!`,'success'); await buildUI(u);
  }catch(e){ showToast(e.message,'danger');}
  toggleLoading($('#btn-log'),false); refreshCaps();
}

/* ---------- dashboard ----------------------------------------------------- */
async function buildUI(u){
  myUsername=u;
  $('#auth-wrap').classList.add('d-none'); $('#dash-wrap').classList.remove('d-none');
  $('#user-slug').textContent=u;
  const link=localStorage.getItem('0k_blob_url')||'#'; $('#blob-link').href=link; $('#blob-link').textContent=link;
  $('#chat-empty').classList.remove('d-none'); $('#chat-pane').classList.add('d-none');
  await loadProfile(); await renderChats(); await renderInvites();
}
...
$('#chat-list').onclick = e=>{ if(e.target.dataset.url) openChat(e.target.dataset.url); };
...
async function openChat(url){
  ...
  $('#chat-empty').classList.add('d-none'); $('#chat-pane').classList.remove('d-none');
...
async function verifyLastTSA(chatUrl){
  ...
  }catch(e){ showToast('Timestamp verify error','danger');}
}
...
let deb=null;
$('#chat-input').oninput = ()=>{ clearTimeout(deb); $('#btn-send').disabled=true; deb=setTimeout(()=>$('#btn-send').disabled=false,CFG.chat_input_debounce_ms); };
$('#btn-send').onclick = async()=>{
  ...
  try{ await sendMessage(currentChat,t); await loadMsgs(); }
  catch(e){ showToast(e.message,'danger');}
...
async function renderInvites(){
  try{
    ...
    // This fetch should use the direct webhook.site URL, NOT allorigins.win, and include the token
    const {invites_log=[]}=await fetch(decodeURIComponent(pin), {
      headers: { '0K-Token': ok0._state.sessionToken }
    }).then(r=>r.json());
...
/* helper – derive keyHex same as p2p */
...
$('#saveNicknameBtn').onclick = async ()=>{
  ...
  try{
    ...
    const raw=await fetch(chatUrl).then(r=>r.text()).catch(()=>null); // This is still direct fetch for JSONBlob
    ...
    // This explicit fetch call will still use CFG.webhook_base_url (with allorigins.win)
...
/* ---------- misc ---------------------------------------------------------- */
window.addEventListener('beforeunload',()=>{ if(currentChat) closeChat(currentChat); });
...
if(isSession()){
  const u=localStorage.getItem('0k_username'); if(u) buildUI(u);
}
...
document.addEventListener('DOMContentLoaded',()=>{
  if(!$('#toast-container')){
    ...
    d.className='position-fixed bottom-0 end-0 p-3'; d.style.zIndex='1050';
