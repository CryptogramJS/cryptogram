...
function flushPending(chat){
  ...
  while (q.length){
    const enc=q.shift();
    fetch(`${CFG.webhook_base_url}/${chat.peerSlug}`,{method:"POST",headers:{'Content-Type':'application/json'},body:JSON.stringify({type:"message",chatUrl:chat.chat_url,payload:enc})}).catch(()=>q.unshift(enc));
  }
}
...
async function updateMyEndpointAll(){
  for(const ch of S.personal.chats){
    ...
    amChange(ch.chat_url,d=>{d.meta=d.meta||{};d.meta[S.myPubKeyHex]={slug:S.hookSlug,http:S.hookUrl,email:S.hookEmail,ts:Date.now()};});
  }
}
...
(async function poll(){
  for(;;){
    ...
    try{
      const url=`${CFG.webhook_base_url}/${S.hookSlug}/requests?min_id=${lastPoll}&sort=asc&limit=20`;
